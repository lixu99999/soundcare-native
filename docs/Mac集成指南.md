...
>
> **推荐流程**：**Phase 0（H5 模拟预验证，需后端）→ Phase 1A（iOS 模拟器，零证书零后端）→ Phase 1B（iPhone 真机）**。
