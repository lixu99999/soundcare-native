...
| # | 问题 | 选项 |
|---|------|------|
| 1 | HRV 采样频率 | A. 系统默认（数据变化即推送）<br>B. 强制 5 秒/次<br>C. 强制 10 秒/次 |
| 2 | 前台/后台监测范围 | A. 仅前台（音频播放时）<br>B. 前台 + 后台持续 |
| 3 | Mock 模式默认 | A. 默认关闭（必须真机）<br>B. 默认开启（开发友好）<br>C. 通过环境变量切换 |
| 4 | 是否支持 iPad | A. 不支持（iPad 无 HealthKit）<br>B. 优雅降级（isAvailable 返回 false） |
| 5 | 音频会话冲突 | 音频播放时是否影响 HealthKit 监测？（实测应不影响）|
| 6 | 错误重试策略 | HealthKit 临时错误是否自动重试？|

**默认选择**（如果用户没异议）：
1 → A，2 → A，3 → A（通过显式 mockMode: true 启用），4 → B，5 → 无需处理，6 → 不自动重试，由上层处理
...
**当前状态**：架构设计完成，待用户 review。
**下一步**：根据 review 反馈写 Swift 代码。
